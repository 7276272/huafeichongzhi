...
function isAgent(obj: Record<string, unknown>): obj is Agent {
  // 只检查最基本的必需字段，确保不会过滤掉数据库有效数据
  return Boolean(obj && typeof obj === 'object' && 'name' in obj && 'email' in obj);
}
...
export const getAgentWithdrawals = async () => {
  try {
    ...
    const { data, error } = await getTable('agent_withdrawals')
      .select('*, agents(id, name, email)')
      .order('created_at', { ascending: false });
    
    if (error) {
      console.error("Error fetching agent withdrawals:", error);
      throw new Error("获取代理提现申请失败");
    }
    
    return data || [];
  } catch (error) {
    console.error("Error in getAgentWithdrawals:", error);
    throw error;
  }
};

// 更新代理提现申请状态
export const updateAgentWithdrawal = async (id: string, updates: Record<string, unknown>) => {
  try {
    // 如果是批准提现操作，使用存储过程
    if (updates.status === 'completed') {
      console.log(`尝试批准提现申请: ${id}`);
      
      // 调用存储过程批准提现
      const { data: result, error: rpcError } = await supabase.rpc('approve_agent_withdrawal', {
        withdrawal_id: id
      });
      
      if (rpcError) {
        console.error("Error in approve_agent_withdrawal RPC:", rpcError);
        throw new Error("处理提现申请失败: " + rpcError.message);
      }
      
      console.log("批准提现成功", result);
      return result;
    } else if (updates.status === 'rejected') {
      // 如果是拒绝提现操作，使用存储过程退回余额
      console.log(`尝试拒绝提现申请: ${id}`);
      
      // 调用存储过程拒绝提现
      const { data: result, error: rpcError } = await supabase.rpc('reject_agent_withdrawal', {
        withdrawal_id: id
      });
      
      if (rpcError) {
        console.error("Error in reject_agent_withdrawal RPC:", rpcError);
        throw new Error("拒绝提现申请失败: " + rpcError.message);
      }
      
      console.log("拒绝提现成功", result);
      return result;
    } else {
      // 其他状态更新
      const { data, error } = await getTable('agent_withdrawals')
      ...
      if (error) {
        console.error("Error updating agent withdrawal:", error);
        throw new Error("更新代理提现申请失败");
      }
      
      return data;
    }
  } catch (error) {
    console.error("Error in updateAgentWithdrawal:", error);
    throw error;
  }
};

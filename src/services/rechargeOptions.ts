...
export const getRechargeOptions = async (): Promise<RechargeOption[]> => {
  try {
    // Try to fetch recharge products from the database with their business type
    const { data: products, error } = await supabase
      .from('recharge_products')
      .select('*, business_types:business_type_id(name)')
      .eq('is_active', true)
      .order('value', { ascending: true });
    
    // If there's an error or no data, use default options
    if (error || !products || products.length === 0) {
      console.log("Using default recharge options", error);
      return defaultRechargeOptions;
    }
    
    console.log("Fetched products:", products);
    
    // Map products to RechargeOption format with product name
    return products.map(product => ({
      id: product.id,
      value: product.value,
      label: `¥${product.value}`,
      name: product.name,
      businessType: product.business_types?.name || '话费充值',
...
export const rechargePhone = async (params: RechargeParams): Promise<RechargeOrder> => {
  try {
    ...
    console.error("Error submitting recharge:", {
    ...
    // 抛出包含更多信息的错误
    throw new Error(`提交充值请求失败: ${errorMessage}`);
  }
};

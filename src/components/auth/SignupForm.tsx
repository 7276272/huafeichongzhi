...
const SignupForm: React.FC<SignupFormProps> = ({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // 如果注册成功且有推广码，调用回调函数记录推广注册
    if (data?.user?.id && referralCode && onRegistrationSuccess) {
      console.log("注册成功，处理推广码:", referralCode);
      try {
        onRegistrationSuccess(data.user.id);
      } catch (error) {
        console.error("处理推广注册失败:", error);
      }
    }

    ...
    try {
      ...
      if (result === 'completed') {
        ...
        const { data: agentData, error: agentError } = await supabase
          .from('agents')
          .select('id, name, commission_rate, balance, total_commission, exchange_rate, usdt_balance')
          .eq('id', agentId)
          .single();
        ...
        if (agentError) {
          ...
          const commissionAmount = (order.amount || 0) * commissionRate;
          const newBalance = (typedAgentData.balance || 0) + commissionAmount;
          const newTotalCommission = (typedAgentData.total_commission || 0) + commissionAmount;
          
          // 计算USDT金额 (订单金额 / 汇率)
          const exchangeRate = typedAgentData.exchange_rate || 7.2; // 默认汇率: 1 USDT = 7.2 RMB
          const usdtAmount = (order.amount || 0) / exchangeRate;
          const newUsdtBalance = (typedAgentData.usdt_balance || 0) + usdtAmount;
          ...
          const { error: updateError } = await supabase
            ...
            .update({
              ...
              usdt_balance: newUsdtBalance  // 更新USDT余额
            })
            .eq('id', agentId);
          ...
          const { error: usdtTransactionError } = await supabase
            .from('agent_usdt_transactions')
            .insert({
              agent_id: agentId,
              order_id: order.id,
              amount: usdtAmount,
              exchange_rate: exchangeRate,
              rmb_amount: order.amount,

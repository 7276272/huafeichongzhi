...
export default function ReferralDashboard({ agent }: ReferralDashboardProps) {
  ...
  const [linkStats, setLinkStats] = useState<{ clicks: number, conversions: number }>({ clicks: 0, conversions: 0 });
  ...
  const loadReferralData = useCallback(async () => {
    ...
    try {
      ...
      // 处理推广码和链接
      let code = agent.referral_code;
      let link = agent.referral_link;

      if (!code || !link) {
        const result = await createAgentReferral(agent.id);
        if (result) {
      ...
      // 生成二维码
      if (link) {
        const qrCode = generateQRCodeURL(link);
        setQrCodeURL(qrCode);

        // 尝试获取已有的短链接
        try {
          const existingShortLink = await getReferralLinkStats(agent.id);
          if (existingShortLink?.shortLink) {
            setShortLink(existingShortLink.shortLink);
          }
          
          // 设置链接统计数据
          if (existingShortLink) {
            setLinkStats({
              clicks: existingShortLink.clicks || 0,
              conversions: existingShortLink.conversions || 0
            });
          }
        } catch (err) {
          console.error("获取短链接统计失败:", err);
          // 如果是表不存在的错误，可能是数据库表还未创建
          const errorObj = err as Error;
          if (errorObj?.message && typeof errorObj.message === 'string' && errorObj.message.includes("does not exist")) {
            console.log("短链接表可能尚未创建，请先运行数据库迁移脚本");
          }
          // 如果获取失败，不阻止其他功能
        }
      }
      ...
      // 加载下级代理统计
      const subAgents = await getSubAgentStats(agent.id);
      setSubAgentStats(subAgents);
  ...
  const copyTextToClipboard = async (text: string): Promise<boolean> => {
    ...
    try {
      ...
      console.warn('选择文本复制失败:', err);
    }

    // 方法3: 使用剪贴板 API（可能受权限限制）
    if (navigator.clipboard && navigator.clipboard.writeText) {
      try {
        await navigator.clipboard.writeText(text);
        return true;
      } catch (err) {
        console.warn('剪贴板 API 失败:', err);
      }
    }
  ...
  const copyReferralLink = async () => {
    try {
      ...
      // 创建短链接（如果还没有）
      if (agent?.id && !shortLink) {
        setGeneratingShortLink(true);
        try {
          const newShortLink = await generateShortLink(referralLink, agent.id);
          setShortLink(newShortLink);
          linkToCopy = newShortLink;
        } catch (error) {
        ...
        linkToCopy = shortLink;
      }
      
      // 使用增强的复制方法
      const success = await copyTextToClipboard(linkToCopy);
      ...
      toast({
        variant: "destructive",
        title: "复制失败",
        description: `无法复制链接: ${errorMessage}`
      });
    }
  };
  ...
  const copyReferralCode = async () => {
    try {
      // 使用增强的复制方法
      const success = await copyTextToClipboard(referralCode);
      ...
      toast({
        variant: "destructive",
        title: "复制失败",
        description: `无法复制推广码: ${errorMessage}`
      });
    }
  };

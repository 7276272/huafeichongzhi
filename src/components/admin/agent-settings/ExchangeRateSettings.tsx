...
interface Agent {
  id: string;
  user_id: string | null;
  commission_rate: number | null; // 佣金率
  exchange_rate: number | null; // 汇率: 1 USDT = ? RMB
...
export default function ExchangeRateSettings() {
  ...
  const fetchAgents = useCallback(async () => {
    ...
    try {
      ...
      // 初始化汇率状态 - 使用exchange_rate字段
      const initialRates: Record<string, number> = {};
      agentsWithUserInfo.forEach(agent => {
        initialRates[agent.id] = agent.exchange_rate || 10.0; // 默认汇率为10.0 (1 USDT = 10.0 RMB)
      });
  ...
  // 保存单个代理的汇率 - 使用exchange_rate字段来存储汇率
  const saveExchangeRate = async (agentId: string) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('agents')
        .update({ exchange_rate: exchangeRates[agentId] })
        .eq('id', agentId);
